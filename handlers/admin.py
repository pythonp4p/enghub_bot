...
async def del_callback_run1(callback_query: types.CallbackQuery):
    await sqlite_db_2.sql_delete_commandd(callback_query.data.replace('dep ', ''))
...
async def del_callback_run(callback_query: types.CallbackQuery):
    await sqlite_db_3.sql_delete_commanddd(callback_query.data.replace('det ', ''))
...
async def delete_item(message: types.Message):
    if message.from_user.id == ID:
            ...
            read1 = await sqlite_db_2.sql_read2()
            read2 = await sqlite_db_3.sql_read2()
            for ret in read:
